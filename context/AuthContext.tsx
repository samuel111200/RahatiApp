...
import AsyncStorage from '@react-native-async-storage/async-storage'; // 👈 ضيف السطر ده هنا
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(raw => {
      if (raw) {
        try { const u = JSON.parse(raw); setUser(u); setIsAuthenticated(true); } catch {}
      }
      setIsLoading(false);
  ...
  const signIn = async (email: string, password: string): Promise<{ ok: boolean; error?: string }> => {
    try {
      ...
      await persist(found.user); return { ok: true };
    } catch { return { ok: false, error: 'حدث خطأ، حاول مرة أخرى' }; }
  };

  const signUp = async (personal: Omit<User, 'email' | 'provider'>, account: { email: string; password: string }): Promise<{ ok: boolean; error?: string }> => {
    try {
      const raw = await AsyncStorage.getItem(ACCOUNTS_KEY);
      const accounts: StoredAccount[] = raw ? JSON.parse(raw) : [];
      ...
      const newUser: User = { ...personal, email: account.email, provider: 'email' };
      accounts.push({ email: account.email, password: account.password, user: newUser });
      await AsyncStorage.setItem(ACCOUNTS_KEY, JSON.stringify(accounts));
      await persist(newUser); return { ok: true };
    } catch { return { ok: false, error: 'حدث خطأ، حاول مرة أخرى' }; }
  };

  const signInWithSocial = async (provider: 'google' | 'facebook', profile: { email: string; firstName: string; lastName: string; photoUrl?: string }) => {
    const socialUser: User = { firstName: profile.firstName, lastName: profile.lastName, age: '', gender: '', email: profile.email, provider, photoUrl: profile.photoUrl };
    const raw = await AsyncStorage.getItem(ACCOUNTS_KEY);
    const accounts: StoredAccount[] = raw ? JSON.parse(raw) : [];
  ...
  const updateProfile = async (data: Partial<User>) => {
    ...
    if (idx >= 0) { accounts[idx].user = updated; await AsyncStorage.setItem(ACCOUNTS_KEY, JSON.stringify(accounts)); }
  };

  return (
    <AuthContext.Provider value={{ user, isAuthenticated, isLoading, signIn, signUp, signInWithSocial, logout, updateProfile }}>
      {children}
