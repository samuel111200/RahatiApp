...
import { db, FSRelationship } from '../../utils/firebaseConfig';
import {
            ...
            <Animated.View style={[styles.actionOverlay, { opacity: opacityAnim }]}>
                ...
                <Animated.View style={[styles.actionSheet, { transform: [{ scale: scaleAnim }] }]}>
                    <View style={styles.actionHeader}>
                        ...
                        <Text style={styles.actionTitle}>{t.docPhotoTitle}</Text>
                        <Text style={styles.actionSubtitle}>{t.docPhotoSubtitle}</Text>
                    </View>
                    ...
                    <TouchableOpacity style={styles.actionBtn} onPress={() => { onClose(); setTimeout(onPickNew, 300); }} activeOpacity={0.7}>
                        ...
                        <Text style={styles.actionBtnText}>{hasAvatar ? t.docChangePhoto : t.docAddPhoto}</Text>
                    ...
                    {hasAvatar && (
                        <>
                            ...
                            <TouchableOpacity style={styles.actionBtn} onPress={() => { onClose(); setTimeout(onDelete, 300); }} activeOpacity={0.7}>
                                ...
                                <Text style={[styles.actionBtnText, { color: Colors.danger }]}>{t.docDeletePhoto}</Text>
                    ...
                    <TouchableOpacity style={[styles.actionBtn, { justifyContent: 'center' }]} onPress={onClose} activeOpacity={0.7}>
                        <Text style={[styles.actionBtnText, { color: Colors.textMuted, textAlign: 'center' }]}>{t.docCancelPhoto}</Text>
                    </TouchableOpacity>
                </Animated.View>
            </Animated.View>
        </Modal>
...
function EditProfileModal({ visible, onClose, user, onSave, t, isRTL }: {
    ...
    const [form, setForm] = useState<DoctorFields>({
        ...
        gender: user?.gender || '', specialty: user?.specialty || '', licenseNumber: user?.licenseNumber || '',
    });
    ...
    const handleSave = async () => {
        if (!form.firstName.trim() || !form.lastName.trim()) {
            Alert.alert(t.docError, t.docNameRequired); return;
        }
        setSaving(true);
        try {
            onSave(form); onClose();
            ...
            Alert.alert(t.docError, t.docSaveFailed);
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 20}>
                ...
                <View style={styles.slideModal}>
                    <View style={[styles.modalHeader, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                        ...
                        <Text style={styles.modalHeaderTitle}>{t.docEditProfileTitle}</Text>
                        <View style={{ width: 36 }} />
                    </View>
                    <ScrollView contentContainerStyle={styles.editFormContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode="none">
                        ...
                        <ProfileField label={t.docSpecialty} value={form.specialty}     onChangeText={handleSpecialty}     isRTL={isRTL} />
                        <ProfileField label={t.docLicense}   value={form.licenseNumber} onChangeText={handleLicenseNumber} keyboardType="numeric" autoCapitalize="none" isRTL={isRTL} />
                        <View style={styles.fieldWrap}>
                        ...
                        <PrimaryButton
                            title={saving ? t.docSaving : t.docSave}
                            onPress={handleSave}
        ...
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <View style={styles.overlay}>
                <View style={styles.modalCard}>
                    ...
                    <Text style={styles.modalTitle}>{t.docLanguage}</Text>
                    <Text style={styles.modalBody}>{t.docSelectLanguage}</Text>
                    ...
                    <OutlineButton title={t.docLangCancel} onPress={onClose} style={{ width: '100%' }} />
                </View>
            </View>
        </Modal>
    );
...
function HelpModal({ visible, onClose, t, isRTL }: {
    ...
    const faqs = [
        { q: t.docFaq1q, a: t.docFaq1a },
        { q: t.docFaq2q, a: t.docFaq2a },
        { q: t.docFaq3q, a: t.docFaq3a },
    ];
            ...
            <SafeAreaView style={styles.slideModalSafe}>
                <View style={styles.slideModal}>
                    <View style={[styles.modalHeader, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                        ...
                        <Text style={styles.modalHeaderTitle}>{t.docHelp}</Text>
                        <View style={{ width: 36 }} />
                    </View>
...
export default function DocMoreScreen() {
    ...
    useEffect(() => {
        // Use user?.uid from React context, NOT auth.currentUser?.uid.
        // auth.currentUser persists until signOut(auth) resolves, so using it
        // means the listener stays alive through logout and gets permission-denied
        // when Firestore revokes the token. user?.uid is set to null by
        // setUser(null) in logout() BEFORE signOut fires, so this effect
        // re-runs and the cleanup (return unsub) runs first.
        const doctorId = user?.uid;
        if (!doctorId) return;
        const q = query(collection(db, 'relationships'), where('doctorId', '==', doctorId));
        const unsub = onSnapshot(q, (snap) => {
            ...
            // Silently ignore permission errors — these fire after logout when
            // the component hasn't unmounted yet but the auth session is gone.
            unsub();
        });
        return unsub;
    }, [user?.uid]);
    ...
    const handlePickAvatar = async () => {
        ...
        if (!permission.granted) {
            Alert.alert(t.docError, t.docPhotoPerm); return;
        }
    ...
    const MENU = [
        { key: 'language', icon: 'language-outline',   label: t.docLanguage, color: DOC_COLOR, value: isRTL ? t.docLanguageAr : t.docLanguageEn },
        { key: 'help',     icon: 'help-circle-outline', label: t.docHelp,     color: '#29B6D4' },
    ];
    ...
    const activePatientsLabel = pendingPatients > 0 ? t.docAcceptedLabel : t.docAcceptedLabel;
    ...
    return (
        <SafeAreaView style={styles.safe}>
            ...
            >
                ...
                <View style={styles.avatarCard}>
                    <View style={styles.docBadge}>
                        ...
                        <Text style={styles.docBadgeText}>{t.docDoctor}</Text>
                    </View>
                    ...
                    <Text style={styles.avatarSpecialty}>{specialty || t.docSpecialtyNotSet}</Text>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.divider} />
                    <InfoRow label={t.docSpecialty} value={specialty}      isRTL={isRTL} />
                    <View style={styles.divider} />
                    <InfoRow label={t.docLicense}   value={licenseNumber}  isRTL={isRTL} />
                    <View style={styles.divider} />
                    <InfoRow
                        label={t.gender}
                ...
                <TouchableOpacity onPress={() => open('logout')}
                    ...
                    <Text style={styles.logoutText}>{t.docLogout}</Text>
                </TouchableOpacity>
            ...
            <EditProfileModal
                visible={activeModal === 'editProfile'} onClose={close}
                user={combinedUser} onSave={handleSaveProfile} t={t} isRTL={isRTL}
            />
            ...
            <Modal visible={activeModal === 'logout'} transparent animationType="fade" onRequestClose={close}>
                <View style={styles.overlay}>
                    <View style={styles.modalCard}>
                        ...
                        <Text style={styles.modalTitle}>{t.docLogout}</Text>
                        <Text style={styles.modalBody}>{t.docLogoutConfirm}</Text>
                        <View style={styles.modalBtns}>
                            <OutlineButton title={t.docLangCancel} onPress={close} style={{ flex: 1 }} />
                            <PrimaryButton
                                title={t.confirm}
