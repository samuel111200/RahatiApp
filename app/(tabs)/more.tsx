// app/(tabs)/more.tsx  –  المزيد / الملف الشخصي
...
const MENU = [
  { key: 'edit',   icon: 'person-outline',       label: 'تعديل الملف الشخصي', color: Colors.primary },
  { key: 'notif',  icon: 'notifications-outline', label: 'الإشعارات',           color: '#F4A32B'     },
  { key: 'lang',   icon: 'language-outline',      label: 'اللغة',               color: '#4CAF82', value: 'English' },
  { key: 'priv',   icon: 'lock-closed-outline',   label: 'الخصوصية والأمان',    color: '#5B9BD5' },
  { key: 'help',   icon: 'help-circle-outline',   label: 'المساعدة والدعم',     color: '#29B6D4' },
] as const;
...
function MenuRow({ icon, label, value, color, onPress, isLast }: { icon: string; label: string; value?: string; color: string; onPress: () => void; isLast: boolean }) {
  return (
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <InfoRow label="الاسم الثاني" value={user?.lastName || ''} />
          <View style={styles.divider} />
          <InfoRow label="البريد الإلكتروني" value={user?.email || ''} />
          <View style={styles.divider} />
          <InfoRow label="العمر" value={user?.age ? `${user.age} سنة` : ''} />
          <View style={styles.divider} />
          <InfoRow label="الجنس" value={user?.gender === 'male' ? 'ذكر' : user?.gender === 'female' ? 'أنثى' : ''} />
        </View>
        ...
        {/* Logout */}
        <TouchableOpacity onPress={() => setLogoutModal(true)} style={styles.logoutBtn} activeOpacity={0.8}>
      ...
      <Modal visible={logoutModal} transparent animationType="fade" onRequestClose={() => setLogoutModal(false)}>
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalBody}>هل أنت متأكد أنك تريد الخروج؟</Text>
            <View style={styles.modalBtns}>
              <OutlineButton title="إلغاء" onPress={() => setLogoutModal(false)} style={{ flex: 1 }} />
              <PrimaryButton title="تأكيد" onPress={() => { setLogoutModal(false); logout(); router.replace('/(auth)/sign-in'); }} style={{ flex: 1, backgroundColor: Colors.danger }} />
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row-reverse', alignItems: 'center', gap: 12, marginBottom: Spacing.xl },
  ...
  avatarCard: {
    ...
    shadowColor: Colors.shadow, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 1, shadowRadius: 12, elevation: 4, position: 'relative',
  },
  ...
  sectionLabel: { fontSize: FontSize.xs, fontWeight: '700', color: Colors.textMuted, textAlign: 'right', letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase' },
  ...
  infoRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: Spacing.base, paddingVertical: 14 },
  infoLabel: { fontSize: FontSize.sm, color: Colors.textSecondary },
  infoValue: { fontSize: FontSize.sm, fontWeight: '600', color: Colors.textPrimary },

  menuRow: { flexDirection: 'row-reverse', alignItems: 'center', paddingHorizontal: Spacing.base, paddingVertical: 14, gap: 12 },
  ...
  logoutBtn: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: Colors.dangerLight, borderRadius: Radius.xl, paddingVertical: 16, marginBottom: Spacing.base },
